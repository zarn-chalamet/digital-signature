            ...
            <div className="grid grid-cols-1 gap-4 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
                {
                    templatesList?.map((template, index) => (
                        <div key={index} className="relative p-4 border rounded-lg bg-slate-50 border-slate-300">
                            <MoreVertical onClick={() => setIsOpenMore(prev => prev === template._id ? null : template._id)} className="absolute cursor-pointer top-7 right-7" />
                            ...
                            <h3 className="mt-3 text-xl font-semibold text-center">{template.title}</h3>
                            {
                                isOpenMore === template._id && <MoreTemplateModal template={template} onAction={onDeleteTemplate} />
                            }
                        </div>
                    ))
                }
